...
export function Tile({ img, name, index, delay = 0 }: { img: string; name: string; index?: number; delay?: number }) {
  return (
    ...
    >
      ...
      <figcaption className="absolute inset-x-[22px] bottom-5 z-[2] flex items-baseline gap-3 font-display text-[clamp(17px,1.3vw,20px)] leading-[1.15] font-semibold tracking-[-0.012em] text-white">
        {name}
      </figcaption>
    </Reveal>
  );
