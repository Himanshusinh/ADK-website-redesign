  ...
  return (
    <section className="theme-dark sec text-ink">
      ...
      <div className="wrap grid items-start gap-[clamp(32px,5vw,80px)] lg:grid-cols-[1.15fr_1fr]">
          ...
          {industries.map((it, i) => (
            <Reveal as="li" key={it.name} delay={i * 0.05}>
              <TLink
                ...
                onFocus={() => setActive(i)}
                className="group grid grid-cols-[1fr_auto] items-center gap-4 border-b border-line py-[clamp(15px,1.6vw,21px)] "
              >
                <span
                  className={clsx(
                ...
                <span
                  className={clsx(
                    'hidden size-10 place-items-center rounded-full transition-[background-color,color,rotate] duration-500 ease-expo lg:grid',
        ...
        <Reveal className="relative hidden aspect-[5/6] overflow-hidden rounded-[28px] bg-paper ring-1 ring-line lg:sticky lg:top-[110px] lg:block">
          ...
          <div className="absolute inset-x-5 bottom-5 flex items-center justify-between rounded-2xl bg-surface/85 px-5 py-4 ring-1 ring-line backdrop-blur-md">
            <span className="font-display text-[17px] font-semibold tracking-[-0.02em]">{industries[active].name}</span>
          </div>
        </Reveal>
      </div>
    </section>
